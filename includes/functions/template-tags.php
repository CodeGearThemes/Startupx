...
if ( ! function_exists( 'startupx_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 */
	function startupx_posted_on() {
		...
		$posted_on = sprintf(
			...
			__( '<span class="date-label"> </span>%s', 'startupx' ),
...
if ( ! function_exists( 'startupx_posted_by' ) ) :
	/**
	 * Prints HTML with meta information for the current author.
	 */
	function startupx_posted_by() {
		// Get the author name; wrap it in a link.
		$byline = sprintf(
			/* translators: %s: post author */
			__( '<span class="author-label">By </span>%s', 'startupx' ),
...
if ( ! function_exists( 'startupx_comment' ) ) :
	/**
	 * Prints HTML with meta information for the comment.
	 */
	function startupx_comment() {
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( '0 Comment<span class="screen-reader-text"> on %s</span>', 'startupx' ),
						array(
							'span' => array(
								'class' => array(),
							),
...
if ( ! function_exists( 'startupx_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function startupx_entry_footer() {
		$startupx_single_tags 	= get_theme_mod( 'startupx-single-tags-enable', true );
		$startupx_single_category = get_theme_mod( 'startupx-single-category-enable', true );

		// Hide category and tag text for pages.
		if ( 'post' === get_post_type() ) {
			if( $startupx_single_category ):
				/* translators: used between list items, there is a space after the comma */
				$categories_list = get_the_category_list( esc_html__( ', ', 'startupx' ) );
				if ( $categories_list ) { ?>
					...
					printf( '<span class="cat-links">%1$s%2$s</span>',
						sprintf( _x( '<span class="cat-text screen-reader-text">Categories</span>', 'Used before category names.', 'startupx' ) ),
						$categories_list
					); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}
			endif;
			
			if( $startupx_single_tags ):
				/* translators: used between list items, there is a space after the comma */
				$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'startupx' ) );
				if ( $tags_list ) { ?>
					...
					/* translators: 1: list of tags. */
						...
						sprintf( _x( '<span class="tags-text screen-reader-text">Tags</span>', 'Used before tag names.', 'startupx' ) ),
						$tags_list
					); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}
			endif;
		...
		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						/* translators: %s: post title */
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'startupx' ),
						array(
							'span' => array(
								'class' => array(),
							),
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'startupx' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if( ! function_exists( 'startupx_single_postmeta' ) ):

	function startupx_single_postmeta(){ 
		$startupx_single_meta_order 	= get_theme_mod( 'startupx-single-meta-order', array( 'startupx-post-date', 'startupx-post-author', 'startupx-post-comments' ) ); ?>
		<div class="entry-meta">
			<?php
				foreach( $startupx_single_meta_order as $key => $value ){

					switch ( $value ) {
						case 'startupx-post-date':
							startupx_posted_on();
						break;
						case 'startupx-post-author':
							startupx_posted_by(); 
						break;
						case 'startupx-post-comments':
							startupx_comment();
							break;
					}
				}
...
if ( ! function_exists( 'startupx_post_thumbnail' ) ) :
	/**
	 ...
	 */
	function startupx_post_thumbnail() {
